...
class CLIP_CA_Module(nn.Module):
    # TODO clip -> ca with diff and gps concat
    def __init__(self,clip_out_dim=512,num_heads=8):
        super().__init__()
        ...
        self.ca = CrossAttention(dim_q=clip_out_dim,dim_kv=kv_dim, num_heads=num_heads)
    def forward(self, patches, patch_ids, valid_mask, gps, diff):
        patch_embs = self.clip(patches, patch_ids, valid_mask) # (B, T, vit_out)
        diff_embs = self.diff_embs(diff) # (B, T, 8)
        gps_embs = self.gps_embs(gps) # (B, T, 16)
        # cross attention
        kv_embs = torch.cat([diff_embs, gps_embs], dim=-1) # (B, T, 24)
        out = self.ca(patch_embs, kv_embs) # (B, T, vit_out)
        return out, gps_embs # (B, T, vit_out), (B, T, 16) for later
