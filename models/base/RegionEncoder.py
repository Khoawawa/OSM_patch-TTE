...
class RegionEncoder(nn.Module):
    ...
    def forward(self, region_features, offset,valid_mask):
        # region_features: [N_total, 1, F,1,1]
        # offset: [N_total, 1, 2]
        # valid_mask: [B, L] bool tensor
        region_features = region_features.squeeze(1).squeeze(-1).squeeze(-1)  # [N_total, F]
        offset = offset.squeeze(1)  # [N_total, 2]
        region_cat = torch.cat([region_features, offset], dim=-1) # [N_total, F+2]
